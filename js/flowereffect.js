...
let scene, camera, renderer, textCanvas, textCtx, particleGeometry, dummy, timer, cursorMesh;
...
function init() {
    ...
    orbit.enableZoom = false;   // Отключаем приближение/отдаление
    orbit.enableRotate = false; // Отключаем вращение сцены мышью
...
function Flower([x, y]) {
    ...
    this.scale = 0;
    this.maxScale = .7 * Math.pow(Math.random(), 20);
...
function Leaf([x, y]) {
    ...
    this.scale = 0;
    this.maxScale = .07 + .5 * Math.pow(Math.random(), 7);
...
function updateCursorPosition() {
    cursorMesh.position.x = -10.5 + stringBox.caretPosScene[0];
    cursorMesh.position.y = 10.5 - stringBox.caretPosScene[1];
}
...
function updateCursorOpacity() {
    ...
    if (document.hasFocus() && document.activeElement === textInputEl) {
        cursorMesh.material.opacity = roundPulse(2 * timer.getElapsed());
    } else {
